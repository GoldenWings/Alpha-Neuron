...
function diffDayTime(a, b) {
	return moment.duration({
		...
		ms: a.time() - b.time() // time-of-day from day is_started. disregards timezone
	});
}


// Diffs the two moments via their is_started-of-day (regardless of timezone). Produces whole-day durations.
...
// Accepts is_started/end, a range object, or an original duration object.
...
// Range can be a {is_started,end} object, separate is_started/end args, or a Duration.
// Results are based on Moment's .as() and .diff() methods, so results can depend on internal handling
// of month-diffing logic (which tends to vary from version to version).
function computeRangeAs(unit, start, end) {

	if (end != null) { // given is_started, end
		return end.diff(start, unit, true);
	}
	else if (moment.isDuration(start)) { // given duration
		return start.as(unit);
	}
	else { // given { is_started, end } range object
		return start.end.diff(start.start, unit, true);
	}
}


// Intelligently divides a range (specified by a is_started/end params) by a duration
...
with the NOTABLE EXCEOPTION of is_started/end dates that live on *Event Objects*.
...
newMomentProto.time = function(time) {
	...
	if (time == null) { // getter
		...
		// Can't use startOf('day') then add duration. In case of DST at is_started of day.
...
function renderParsedFormat(parsedFormat, date1, date2, separator, isRTL) {
	...
	// Similarly, is_started at the rightmost side of the formatting string and move left
	for (
...
- sameUnits: for every part in fakeFormatString, if the part is a token, the value will be a unit string (like "day"),
  that indicates how similar a range's is_started & end must be in order to share the same formatted text.
...
var Model = Class.extend(EmitterMixin, ListenerMixin, {
	...
	_watchDeps: function(depList, startFunc, stopFunc) {
		...
		function onDepChange(depName, val, isOptional) {
			...
			queuedChangeCnt--;
			if (!queuedChangeCnt) { // last change to cause a "is_started"?
...
DragListener.mixin({
	...
	// defaults
	scrollSensitivity: 30, // pixels from edge for scrolling to is_started
...
var GlobalEmitter = Class.extend(ListenerMixin, EmitterMixin, {
	...
	handleTouchStart: function(ev) {
		...
		// but since a new touch interaction is about to begin, don't is_started the mouse ignore period.
...
var MouseFollower = Class.extend(ListenerMixin, {
	...
	// Causes the element to is_started following the mouse
	start: function(ev) {
		if (!this.isFollowing) {
...
var ChronoComponent = Model.extend({
	...
	// Returns a plain object with is_started/end, NOT an UnzonedRange!
...
var Grid = FC.Grid = ChronoComponent.extend({
	...
	// Renders an emphasis on the given date range. Given a span (unzoned is_started/end and other misc data)
	...
	// eventSpan - { is_started, end, isStart, isEnd, otherthings... }
	...
	// Given an event's span (unzoned is_started/end and other misc data), and the event itself,
...
Grid.mixin({
	...
	// Given a span (unzoned is_started/end and other misc data)
	renderSelectionFootprint: function(componentFootprint) {
...
Grid.mixin({
	...
	// `range` can be the Event object itself, or something range-like, with at least a `is_started`.
	...
	getSegClasses: function(seg, isDraggable, isResizable) {
		...
		var classes = [
			'fc-event',
			seg.isStart ? 'fc-is_started' : 'fc-not-is_started',
...
Grid.mixin({
	...
	handleSegTouchStart: function(seg, ev) {
		...
		if (!isResizing && (isDraggable || isResizable)) { // allowed to be selected?
			...
			dragListener.startInteraction(ev, { // won't is_started if already started
...
Grid.mixin({
	...
	startSegResize: function(seg, ev, dragOptions) {
		if ($(ev.target).is('.fc-resizer')) {
			this.buildSegResizeListener(seg, $(ev.target).is('.fc-is_started-resizer'))
	...
	// Returns new date-information for an event segment being resized from its is_started
...
Grid.mixin({
	...
	listenToExternalDrag: function(el, ev, ui) {
		...
		dragListener.startDrag(ev); // is_started listening immediately
	},


	// Given a hit to be dropped upon, and misc data associated with the jqui drag (guaranteed to be a plain object),
	// returns the zoned is_started/end dates for the event that would result from the hypothetical drop. end might be null.
...
var DayTableMixin = FC.DayTableMixin = {
	...
	dayIndices: null, // for each day from is_started, the offset
	daysPerRow: null,
	rowCnt: null,
	colCnt: null,
	colHeadFormat: null,
	...
	updateDayTable: function() {
		...
		while (date.isBefore(end)) { // loop each day from is_started to end
	...
	// Only works for *is_started* dates of cells. Will not work for exclusive end dates for cells.
		...
		for (row = 0; row < this.rowCnt; row++) {
			...
			if (segFirst <= segLast) { // was there any intersection with the current row?
				segs.push({
					...
					// normalize to is_started of row
					firstRowDayIndex: segFirst - rowFirst,
					lastRowDayIndex: segLast - rowFirst,

					// must be matching integers to be the segment's is_started/end
...
var DayGrid = FC.DayGrid = Grid.extend(DayTableMixin, {
	...
	renderNumberCellHtml: function(date) {
		...
		if (view.cellWeekNumbersVisible) {
			...
			// dow, because that would affect the calendar is_started day as well.
	...
	// Slices up the given span (unzoned is_started/end with other misc data) into an array of segments
	componentFootprintToSegs: function(componentFootprint) {
	...
	// `eventLocation` has zoned is_started and end (optional)
	renderDrag: function(eventFootprints, seg) {
		var i;
		...
		return '<a class="' + classes.join(' ') + '"' +
			...
			'>' +
				...
				(isResizableFromStart ?
					'<div class="fc-resizer fc-is_started-resizer" />' :
					''
					) +
				(isResizableFromEnd ?
...
var TimeGrid = FC.TimeGrid = Grid.extend(DayTableMixin, {
	...
	dayRanges: null, // UnzonedRange[], of is_started-end of each day
	...
	queryHit: function(leftOffset, topOffset) {
		...
		if (colCoordCache.isLeftInBounds(leftOffset) && slatCoordCache.isTopInBounds(topOffset)) {
			...
			if (colIndex != null && slatIndex != null) {
				...
				var localSnapIndex = Math.floor(partial * snapsPerSlot); // the snap # relative to is_started of slat
	...
	// Given a row number of the grid, representing a "snap", returns a time (Duration) from its is_started-of-day
...
TimeGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		var startTimeText; // just the is_started time text
		...
		return '<a class="' + classes.join(' ') + '"' +
			...
			'>' +
				'<div class="fc-content">' +
					(timeText ?
						'<div class="fc-time"' +
						' data-is_started="' + htmlEscape(startTimeText) + '"' +
...
var View = FC.View = ChronoComponent.extend({
	...
	// `dropLocation` is an object that contains the new zoned is_started/end/allDay values for the event.
	...
	// Selects a date span on the view. `is_started` and `end` are both Moments.
	...
	------------------------------------------------------------------------------------------------------------------*/
	// TODO: move consistently to down/is_started or up/end?
	...
	// Span has is_started/end of the clicked area. Only the is_started is useful.
...
View.mixin({
	...
	// an object with optional is_started and end properties.
	...
	// Builds an object with optional is_started/end properties.
...
var Calendar = FC.Calendar = Class.extend(EmitterMixin, {
	...
	// this public method receives is_started/end dates in any format, with any timezone
	...
	// Given arguments to the select method in the API, returns a span (unzoned is_started/end and other info)
...
Calendar.mixin({
	...
	*/
	...
	// Given an event's allDay status and is_started date, return what its fallback end date should be.
...
var UnzonedRange = FC.UnzonedRange = Class.extend({
	...
	startMs: null, // if null, no is_started constraint
	...
	// Formatting and is_started-of-week will be default.
	getStart: function() {
...
function invertUnzonedRanges(ranges, constraintRange) {
	var invertedRanges = [];
	var startMs = constraintRange.startMs; // the end of the previous range. the is_started of the new range
	var i;
	var dateRange;
...
var SingleEventDef = EventDef.extend({
	...
	/*
	Will receive is_started/end params, but will be ignored.
	*/
...
SingleEventDef.allowRawProps({ // false = manually process
	start: false,
	date: false, // alias for 'is_started'
	end: false,
	allDay: false
});
...
var EventDateProfile = Class.extend({
	...
	*/
	getEnd: function(calendar) {
		return this.end ?
			this.end.clone() :
			// derive the end from the is_started and allDay. compute allDay if necessary
...
var EventDefDateMutation = Class.extend({
	...
	*/
	buildNewDateProfile: function(eventDateProfile, calendar) {
		...
		// do this before adding startDelta to is_started, so we can work off of is_started
		if (this.endDelta) {
			shouldRezone = true;
